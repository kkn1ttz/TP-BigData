...
* Relier les processeurs : `ScanHBase` -> `ConvertRecord` -> `PutHDFS`
...
* Configurer `Scheduling` du processor `ScanHBase`
...
* Paramétrer :
  ...
  * `Table Name` = people\_hbase
  * `Columns` = info\:first\_name, info\:last\_name, info\:city
  * `Encoding Strategy` = None
  * `HBase Client Service` = le `HBaseClientService` créé ci-dessus
